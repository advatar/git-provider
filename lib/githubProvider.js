...
GithubProvider.prototype.authenticatedUserGet = function(callback) {
...
GithubProvider.prototype.repoCreate = function(repoName, callback) {
...
GithubProvider.prototype.repoDelete = function(repoName, callback) {
  this._getUserRepo(repoName).deleteRepo(function(err, repo) {
    callback(err, repo);
  });
}

GithubProvider.prototype.repoFork = function(opts, callback) {
  var match = opts.url.match(/^git:\/\/github.com\/(.+)\/(.+).git$/);
...
GithubProvider.prototype.repoGetContents = function(opts, callback) {
